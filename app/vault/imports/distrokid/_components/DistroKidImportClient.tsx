...
export default function DistroKidImportClient() {
  ...
  async function createSession() {
    setLoading(true);
    try {
      const response = await fetch("/api/distrokid-import/session", {
        method: "POST",
  ...
  async function checkStatus() {
    ...
    try {
      const response = await fetch(`/api/distrokid-import/status/${encodeURIComponent(session.session_token)}`, {
        method: "GET",
        cache: "no-store",
      });
